...
void KyAutoMatch::with_Constraint_doit()
{
	...
	// bIndex: 정규 데이타의 인덱스
	// bData: 정규 데이타 포인트
	// nMeasured: 계측 데이타의 갯수
	// mIndex: 계측 데이타의 인덱스
	// mData: 계측 데이타 포인트
	// nconstraint: constraint의 갯수
	// cIndex: 구속조건이 부여되는 점들의 인덱스
	// cLevel: 구속 조건의 레벨
	// nfiltered: 정합된 점들의 갯수 (필터링이 된 이후의 점의 갯수)
	// oTransformed: 정합된 계측 점. 새로운 변환 점들은 함수값으로 돌려 받는다
	// filteringIndex: 각 계측값과 매칭되는 정규값의 인덱스
	// dist: 각 대응 점들간의 최소 거리
	// filteredYN: 필터링 여부
...
//   }
  ...
  *nfiltered = nMeasured;
  ...
  updateCorres(bData,nBase,mRotatedData,nMeasured,distData,filteringindex,delta,filteredYN);	/* 추가부분 10/2/2007 */
