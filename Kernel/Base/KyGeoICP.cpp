...
void KyGeoICP::SetSource(const KyP3Ds& source)
{
	m_Source = source;
}

void KyGeoICP::SetTarget(const KyP3Ds& target)
{
	m_Target = target;
}

void KyGeoICP::Set(const KyP3Ds& source, const KyP3Ds& target)
{
	SetSource(source);
	SetTarget(target);
}
...
bool KyGeoICP::GetRigidTM(KyTMatrix& tm, const int& iter, const double& tol, bool errorMetric,bool bMappingFixX, bool bMappingFixY,bool bMappingFixZ)
{
	...
	for (int i = 0; i < iter; i++) {
		...
		if (error0 < error1) {// 
			if (i > 0)
				improved = true;
			KyTMatrix inverseM = tm.InverseM();
...
KyTMatrix KyGeoICP::CalcRigidTM(bool bMappingFixX/* = false*/,	bool bMappingFixY /*= false*/,	bool bMappingFixZ /*= false*/) const
{ 
	KyTMatrix tm;
	tm.MakeIdentity();
	...
	KyStdArray<KyP3D>::const_iterator sit;
	KyStdArray<KyP3D>::const_iterator tit;
	KyP3D a, b;
...
// each other.
void KyGeoICP::Perpendiculars(const KyV3D& v0, 
...
bool KyGeoICP::TransformSource(const KyTMatrix& tm)
{	
	KyStdArray<KyP3D>::iterator sit;
